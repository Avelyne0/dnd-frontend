...
export default class CharacterGenerator extends Component {
  ...
  setCharacterAlignmentBackgroundDeity = () => {
    ...
    const validIdealArray = background.ideals.filter(
      ideal =>
        alignmentSplit.includes(ideal[0].alignment.toLocaleLowerCase()) ||
        "any" === ideal[0].alignment.toLocaleLowerCase()
    );
    const validIdeals = validIdealArray.map(array => array[1].ideal);
    const filteredDeitiesArray = data.deities.filter(deity =>
      alignmentSplit.includes(deity[1].alignment.toLocaleLowerCase())
    );
  ...
  setCharacterStatsRaceSkills = () => {
    ...
    const skin = random(race.physicalCharacteristics[0].skin);
    // var hair = ''
    ...
    this.setState({ ...this.state.additionalNotes, additionalNotes });
  };
